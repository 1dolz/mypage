...
const DEFAULT_FIELD_MAP = {
  ...
  purchases: 'Purchases,구매,구매수,전환수,Conversions,Results',
  revenue: 'Revenue,매출,전환매출액,매출액,Purchase value,Conversion value,Sales',
};
...
function parseManualUploadFile(buffer, ext, sourceConfig, opts = {}) {
  ...
  return records.map((r) => {
    ...
    return {
      ...
      purchases: toNumber(pickField(r, effectiveAltNames(fieldMap, defaultsMap, 'purchases'))),
      revenue: toNumber(pickField(r, effectiveAltNames(fieldMap, defaultsMap, 'revenue'))),
      extra: r,
    };
  });
}
