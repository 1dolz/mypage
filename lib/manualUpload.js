...
// 사용자가 지정한 재정의 값을 먼저 넣고, 그다음 기본 후보명을 이어붙임 (중복 제거).
function effectiveAltNames(fieldMap, key) {
  const override = splitAltNames(fieldMap && fieldMap[key]);
  const fallback = splitAltNames(DEFAULT_FIELD_MAP[key]);
  return [...new Set([...override, ...fallback])];
}
...
function findHeaderRow(aoa, fieldMap) {
  const allNames = new Set(FIELD_KEYS.flatMap((key) => effectiveAltNames(fieldMap, key)));
  let bestIdx = 0;
  let bestScore = -1;
...
function parseManualUploadFile(buffer, ext, sourceConfig, opts = {}) {
  ...
  return records.map((r) => {
    const rawCost = toNumber(pickField(r, effectiveAltNames(fieldMap, 'cost')));
    let cost = rawCost * costMultiplier;
    if (sourceConfig.apply_margin_rate) {
      cost = cost / (marginRate || 1);
    }
    const adNameRaw = pickField(r, effectiveAltNames(fieldMap, 'ad_name'));

    return {
      date: parseFlexibleDate(pickField(r, effectiveAltNames(fieldMap, 'date'))),
      campaign_name: String(pickField(r, effectiveAltNames(fieldMap, 'campaign_name')) || ''),
      adgroup_name: String(pickField(r, effectiveAltNames(fieldMap, 'adgroup_name')) || ''),
      ad_name: adNamePrefix ? `${adNamePrefix}${adNameRaw || ''}` : String(adNameRaw || ''),
      cost: Math.round(cost),
      impressions: toNumber(pickField(r, effectiveAltNames(fieldMap, 'impressions'))),
      clicks: toNumber(pickField(r, effectiveAltNames(fieldMap, 'clicks'))),
      installs: toNumber(pickField(r, effectiveAltNames(fieldMap, 'installs'))),
      extra: r,
    };
  });
}
