...
const MEDIA_SETTINGS_SCHEMA = [
  {
    ...
    label: 'Meta',
    fields: [
      { key: 'META_APP_ID', label: 'App ID', tip: 'Meta for Developers에서 만든 앱의 App ID입니다. 발급 절차는 <a href="/guide#media">가이드</a> 참고.' },
      { key: 'META_APP_SECRET', label: 'App Secret', type: 'password', tip: '같은 앱의 App Secret. 외부에 노출되지 않게 주의하세요.' },
      { key: 'META_AD_ACCOUNT_ID', label: 'Ad Account ID (act_로 시작)', tip: 'Ads Manager의 광고 계정 ID. <b>act_</b> 로 시작하는 전체 문자열을 넣으세요.' },
      { key: 'META_ACCESS_TOKEN', label: 'Access Token', type: 'password', tip: '단기 토큰을 넣으면 최초 실행 때 <b>장기 토큰으로 자동 교환</b>됩니다. 만료되면 새로 발급해 다시 넣으세요.' },
      {
  ...
  },
  {
    ...
    label: 'Google Ads',
    fields: [
      { key: 'GOOGLE_DEVELOPER_TOKEN', label: 'Developer Token', type: 'password', tip: 'Google Ads → 도구 → <b>API 센터</b>에서 발급하는 토큰입니다.' },
      { key: 'GOOGLE_CLIENT_ID', label: 'Client ID', tip: 'Google Cloud 콘솔에서 만든 OAuth 클라이언트의 ID입니다.' },
      { key: 'GOOGLE_CLIENT_SECRET', label: 'Client Secret', type: 'password', tip: '같은 OAuth 클라이언트의 Secret.' },
      { key: 'GOOGLE_CUSTOMER_ID', label: 'Customer ID', tip: '데이터를 가져올 광고 계정 ID. 하이픈 없이 숫자만.' },
      { key: 'GOOGLE_MCC_ID', label: 'MCC ID', tip: '상위 관리자(MCC) 계정 ID. 하이픈 없이 숫자만.' },
      { key: 'GOOGLE_REFRESH_TOKEN', label: 'Refresh Token', type: 'password', tip: '아래 <b>Refresh Token 발급</b>으로 자동 생성됩니다. 직접 입력하지 않아도 돼요.' },
    ],
  },
  {
    id: 'tiktok',
    label: 'TikTok',
    fields: [
      { key: 'TIKTOK_APP_ID', label: 'App ID', tip: 'TikTok for Business → 개발자 앱의 App ID입니다.' },
      { key: 'TIKTOK_APP_SECRET', label: 'Secret', type: 'password', tip: '같은 앱의 Secret.' },
      { key: 'TIKTOK_AUTH_CODE', label: 'Auth Code (최초 발급용, 1회용)', type: 'password', tip: '1회용 인증 코드. App ID/Secret을 먼저 저장한 뒤 아래 <b>Access Token 발급</b> 버튼을 누르면 토큰·Advertiser ID가 채워집니다. 실패하면 새로 발급받아 다시 저장.' },
      { key: 'TIKTOK_ACCESS_TOKEN', label: 'Access Token', type: 'password', tip: '아래 발급 버튼으로 자동 채워집니다. 직접 입력하지 않아도 돼요.' },
      { key: 'TIKTOK_ADVERTISER_ID', label: 'Advertiser ID', tip: '아래 발급 버튼으로 자동 채워집니다.' },
    ],
  },
  {
    id: 'apple',
    label: 'Apple',
    fields: [
      { key: 'APPLE_CLIENT_ID', label: 'Client ID', tip: 'Apple Search Ads → 계정 설정 → API에서 발급하는 값들입니다.' },
      { key: 'APPLE_TEAM_ID', label: 'Team ID' },
      { key: 'APPLE_KEY_ID', label: 'Key ID' },
      { key: 'APPLE_ORG_ID', label: 'Org ID', tip: 'Search Ads 조직(계정) ID입니다.' },
      { key: 'APPLE_PRIVATE_KEY', label: 'Private Key (.p8 파일 내용 전체)', type: 'textarea', tip: '.p8 키 파일을 텍스트로 열어 <b>-----BEGIN</b> 부터 <b>END-----</b> 까지 통째로 붙여넣으세요.' },
    ],
  },
  {
    id: 'adpopcorn',
  ...
  },
  {
    ...
    label: '네이버',
    fields: [
      { key: 'NAVER_API_KEY', label: 'API Key', tip: '네이버 검색광고 → 도구 → <b>API 사용 관리</b>에서 발급합니다. (API Key / Secret Key / Customer ID)' },
      { key: 'NAVER_SECRET_KEY', label: 'Secret Key', type: 'password', tip: '같은 화면의 비밀키. 발급 시 한 번만 보이니 잘 보관하세요.' },
      { key: 'NAVER_CUSTOMER_ID', label: 'Customer ID', tip: '네이버 검색광고 계정의 Customer ID (숫자).' },
    ],
  },
];
